...
(define 
    ...
    (:action pick_up
        ...
        :precondition (and
            ...
            (be_at_box ?b ?l)
            (forall (?b1 - box)
                (not_assigned_robot_box ?b1 ?r)
            )
            ; no robot should carry this
            (forall (?r1 - robot)
                (not_assigned_robot_box ?b ?r1)
            )            
        )
    ...
    ; move with box carried having a content inside
    (:action move_full_wc
    ...
    ; move with box carried not having a content inside
