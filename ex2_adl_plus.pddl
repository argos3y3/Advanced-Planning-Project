...
; BRIEF DESCRIPTION:
; Domain for ex2 of APTP exam project, numerical version
; PLANNING VERSION: Classical planning, PDDL 1.0
; REQUIRE: strips + typing + equality + adl 
...
(define 
    ...
    (:requirements 
        ...
        :fluents  ;; numeric fluents handling
    )
    ...
    (:functions
      	( carrier_capacity ?c - carrier)  ;; use to measure carrier capacity
    )
    ...
    (:action fill_box
        ...
        :precondition (and
            ...
            (forall (?c1 - content)    
                (not_assigned_box_content ?b ?c1)
            )
            ; no box assigned to content
            (forall (?b1 - box) 

                (not_assigned_box_content ?b1 ?c)
            )
            ...
            (forall (?cr1 - carrier) 
            	(not_assigned_carrier_box ?b ?cr1)
            )      
        )
        :effect (and
    ...
    (:action pick_up
        ...
        :precondition (and
            ...
            (> (carrier_capacity ?cr ) 0)   ;; check if capacity is greater than 0
        )
        :effect (and
            ...
            ; decrease capacity
            (decrease (carrier_capacity ?cr ) 1) 
        )     
    )
    
    ; deliver with carry capacity at least less than 3
    (:action drop
        :parameters (?b - box ?cr - carrier ?l - generic_location ?r - robot)
        :precondition (and
            ...
            (assigned_carrier_box ?b ?cr )            
            ; parte carrier
            (< (carrier_capacity ?cr) 4)  ;; should have at least a box carried
        )
        :effect (and
            ...
            (not_assigned_carrier_box ?b ?cr)
            ; parte carrier
            (increase (carrier_capacity ?cr) 1)
    ...
    (:action move_cc_4
        :parameters (?r - robot ?l1 - generic_location ?l2 - generic_location ?cr - carrier)
        :precondition (and
